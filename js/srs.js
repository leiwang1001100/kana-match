...
// Priority: 1) due cards (overdue first)  2) new cards  3) random future card
// keyFn: optional function(romaji) => storageKey (for script-aware SRS)
function pickNextCard(cards, deck, keyFn) {
  const key = keyFn || (r => r);
  const t = today();

  // 1) Due / overdue cards — pick most overdue first
  const due = deck.filter(it => {
    const c = cards[key(it.romaji)];
    return c && c.dueDate !== null && c.dueDate <= t;
  });
  if (due.length > 0) {
    due.sort((a, b) => cards[key(a.romaji)].dueDate.localeCompare(cards[key(b.romaji)].dueDate));
    // Pick randomly among the top overdue (within same date) to add variety
    const topDate = cards[key(due[0].romaji)].dueDate;
    const sameDate = due.filter(it => cards[key(it.romaji)].dueDate === topDate);
    return sameDate[Math.floor(Math.random() * sameDate.length)];
  }

  // 2) New cards (never seen)
  const newCards = deck.filter(it => !cards[key(it.romaji)] || cards[key(it.romaji)].dueDate === null);
  if (newCards.length > 0) {
    return newCards[Math.floor(Math.random() * newCards.length)];
  }

  // 3) All cards are future — pick randomly to avoid always showing the same card
  const future = deck.filter(it => cards[key(it.romaji)] && cards[key(it.romaji)].dueDate !== null);
  return future[Math.floor(Math.random() * future.length)];
}

// Count how many cards in the deck are due today
// keyFn: optional function(romaji) => storageKey
function countDue(cards, deck, keyFn) {
  const key = keyFn || (r => r);
  const t = today();
...
// keyFn: optional function(romaji) => storageKey
function countNew(cards, deck, keyFn) {
  const key = keyFn || (r => r);
  return deck.filter(it => !cards[key(it.romaji)] || cards[key(it.romaji)].dueDate === null).length;
}
